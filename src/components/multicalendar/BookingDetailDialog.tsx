...
import { 
  CalendarIcon, Clock, User, FileText, Loader2, AlertCircle, 
  Check, X, Trash2, Edit, AlertTriangle, CheckCircle2, Ban, HardHat
} from "lucide-react";
...
export function BookingDetailsDialog({
  ...
  const isMyBooking = user?.role === "subcontractor" && user?.id === data?.subcontractor_id;
  ...
  const openConfirm = (type: "deny" | "cancel" | "delete" | "confirm" | "complete") => {
  ...
  return (
    <>
        ...
        <DialogContent className="max-w-lg bg-white p-0 gap-0 border-slate-200 shadow-2xl overflow-hidden">
          ...
          <DialogHeader className="px-6 pt-6 pb-4 bg-white border-b border-slate-50">
            <div className="flex items-start justify-between gap-4">
               ...
               {!loading && data && (
                <Badge className={`${getStatusColor(data.status)} capitalize border px-3 py-1 text-xs font-semibold shadow-none rounded-full`}>
                  {data.status}
                </Badge>
              )}
            </div>
            {!loading && data?.asset && (
                <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-sm font-medium text-slate-900 bg-slate-100 px-2 py-0.5 rounded-md">
                        {data.asset.name}
                        <span className="text-slate-400 ml-1 font-normal">({data.asset.asset_code})</span>
                    </span>
                </div>
            )}
          </DialogHeader>
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-1.5">
                    ...
                    <p className="text-sm font-medium text-slate-900">
                      {format(new Date(data.booking_date), "EEEE, MMMM d, yyyy")}
                    </p>
                  </div>
                  ...
                  {/* Manager (Booked By) */}
                  {data.manager && (
                    <div className={`space-y-2 ${data.subcontractor ? 'col-span-2 sm:col-span-1' : 'col-span-2'}`}>
                      <div className="flex items-center gap-2 text-xs font-bold text-slate-400 uppercase tracking-wider">
                      ...
                      <div className="flex items-center gap-3 bg-slate-50 p-2 rounded-md border border-slate-100">
                        ...
                        <div className="min-w-0">
                            ...
                            <p className="text-xs text-slate-500 truncate">{data.manager.email}</p>
                        </div>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="space-y-2 col-span-2 sm:col-span-1">
                      ...
                      <div className="flex items-center gap-3 bg-orange-50/50 p-2 rounded-md border border-orange-100">
                        <div className="h-8 w-8 rounded-full bg-orange-600 text-white text-xs flex items-center justify-center font-bold ring-2 ring-white shadow-sm shrink-0">
                          {data.subcontractor.company_name?.[0] || data.subcontractor.first_name?.[0]}
                        </div>
                        <div className="min-w-0">
                            <p className="text-sm font-semibold text-slate-900 truncate">
                                {data.subcontractor.company_name || `${data.subcontractor.first_name} ${data.subcontractor.last_name}`}
                            </p>
                            <p className="text-xs text-slate-500 truncate">Subcontractor</p>
                        </div>
                      </div>
                    </div>
                  )}
            ...
            <div className="bg-slate-50 p-4 border-t border-slate-100">
                <div className="flex flex-col-reverse sm:flex-row sm:justify-between w-full gap-3">
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                        ...
                        {(status === "pending" || status === "confirmed") && (
                            <>
                                {hasManagerPrivileges && status === "pending" && (
                                    <Button variant="ghost" className="text-red-600 hover:text-red-700 hover:bg-red-50 justify-start" onClick={() => openConfirm("deny")}>
                                        <Ban className="h-4 w-4 mr-2" /> Reject
                                    </Button>
                                )}
                                {(hasManagerPrivileges || isMyBooking) && status === "confirmed" && (
                                    <Button variant="ghost" className="text-red-600 hover:text-red-700 hover:bg-red-50 justify-start" onClick={() => openConfirm("cancel")}>
                                        <X className="h-4 w-4 mr-2" /> Cancel Booking
                                    </Button>
                                )}
                                {isMyBooking && !hasManagerPrivileges && status === "pending" && (
                                    <Button variant="ghost" className="text-red-600 hover:text-red-700 hover:bg-red-50 justify-start" onClick={() => openConfirm("cancel")}>
                        ...
                        {/* Delete Logic */}
                        {(status === "cancelled" || status === "denied") && isMyBooking && (
                             <Button variant="ghost" className="text-red-600 hover:text-red-700 hover:bg-red-50 justify-start" onClick={() => openConfirm("delete")}>
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                        ...
                        <Button variant="ghost" onClick={onClose} className="sm:hidden text-slate-500">Close</Button>

                        {(status === "pending" || status === "confirmed") && (
                            <>
                                {/* Reschedule is always available for active bookings */}
                                {(hasManagerPrivileges || isMyBooking) && (
                                     <Button variant="outline" className="bg-white border-slate-200 text-slate-700 hover:bg-slate-50 justify-start sm:justify-center" onClick={() => setIsRescheduleOpen(true)}>
                                ...
                                {/* Primary Actions */}
                                {hasManagerPrivileges && status === "pending" && (
                                    <Button className="bg-[#0B1120] hover:bg-[#1a253a] text-white justify-start sm:justify-center" onClick={() => openConfirm("confirm")}>
                                        <Check className="h-4 w-4 mr-2" /> Approve Request
                                    </Button>
                                )}
                                {hasManagerPrivileges && status === "confirmed" && (
                                    <Button className="bg-blue-600 hover:bg-blue-700 text-white justify-start sm:justify-center" onClick={() => openConfirm("complete")}>
      ...
      <AlertDialog 
        ...
        onOpenChange={(open) => setConfirmAction({ ...confirmAction, isOpen: open })}
      >
        <AlertDialogContent className="bg-white">
            <AlertDialogHeader>
                <AlertDialogTitle className="flex items-center gap-2">
                    {confirmAction.type === "confirm" || confirmAction.type === "complete" ? (
                ...
                <AlertDialogDescription className="text-slate-600">
                    {confirmAction.type === "confirm" && "This will confirm the booking time and notify the user."}
                    {confirmAction.type === "complete" && "This will mark the job as finished and archive it."}
                    {confirmAction.type === "deny" && "The user will be notified that their request was rejected."}
                    {confirmAction.type === "cancel" && "Are you sure you want to cancel this booking? The slot will become available again."}
            ...
            <AlertDialogFooter className="mt-2">
                ...
                <AlertDialogAction 
                    ...
                    className={`
                        ${(confirmAction.type === "deny" || confirmAction.type === "cancel" || confirmAction.type === "delete") 
                            ? "bg-red-600 hover:bg-red-700" 
                            : "bg-[#0B1120] hover:bg-[#1a253a]"} text-white
                    `}
                    onClick={(e) => {
                        e.preventDefault();
                        if (confirmAction.type === "delete") handleDelete();
                        else if (confirmAction.type === "confirm") handleUpdateStatus("confirmed");
                        else if (confirmAction.type === "complete") handleUpdateStatus("completed");
                        else if (confirmAction.type === "deny") handleUpdateStatus("denied");
                        else if (confirmAction.type === "cancel") handleUpdateStatus("cancelled");
                    }}
                >
                    {actionLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : "Yes, Proceed"}
                </AlertDialogAction>
            </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
